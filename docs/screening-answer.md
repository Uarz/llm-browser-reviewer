...
**Verification:** The Playwright dry-run path is verified locally and through GitHub Actions. A real LLM run has also been completed successfully through an OpenAI-compatible relay using `OPENAI_BASE_URL` and `gpt-5.4-mini`. Secrets can be provided through environment variables or a local `.env` file, which is intentionally ignored by Git.
